...
## Admin area (Phase 4, `src/routes/admin.ts` + `public/admin.*`)
...
- Admin passwords are stored as salted **PBKDF2** hashes (Web Crypto) in
  `admin_users` — never plaintext. `src/lib/admin.ts` has `hashPassword`/
  `verifyPassword`, `issueAdminSession`, `requireAdmin`, enroll-code helpers.
- `admin_users` session linkage: admin sessions store the admin's `id` in the
  `sessions.user_id` column (added in migration 0003). `/me` and self-guards
  resolve the admin via `session.user_id ?? session.id`.
- Enrollment: farmer `POST /api/auth/enroll` now REQUIRES a code — either a
  per-user D1 `enroll_codes` code (hashed in DB, plaintext shown once on create)
  or the legacy static `ENROLL_KEY` secret. Codes support `expires_at`/`max_uses`
  and can be revoked. There is no open-enrollment fallback.
- Manage via `/api/admin/users` (CRUD), `/api/admin/enroll-codes` (CRUD),
  `/api/admin/sessions` (list + `POST /:id/revoke` to force-logout a device).
